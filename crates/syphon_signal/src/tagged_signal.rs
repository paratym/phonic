...
impl TaggedSignalReader {
    ...
    pub fn pipe(self, writer: &mut TaggedSignalWriter) -> Result<u64, SyphonError> {
        match (self, writer) {
            (Self::I8(mut r), TaggedSignalWriter::I8(w)) => w.write_all(&mut r),
            (Self::I16(mut r), TaggedSignalWriter::I16(w)) => w.write_all(&mut r),
            (Self::I32(mut r), TaggedSignalWriter::I32(w)) => w.write_all(&mut r),
            (Self::I64(mut r), TaggedSignalWriter::I64(w)) => w.write_all(&mut r),
            (Self::U8(mut r), TaggedSignalWriter::U8(w)) => w.write_all(&mut r),
            (Self::U16(mut r), TaggedSignalWriter::U16(w)) => w.write_all(&mut r),
            (Self::U32(mut r), TaggedSignalWriter::U32(w)) => w.write_all(&mut r),
            (Self::U64(mut r), TaggedSignalWriter::U64(w)) => w.write_all(&mut r),
            (Self::F32(mut r), TaggedSignalWriter::F32(w)) => w.write_all(&mut r),
            (Self::F64(mut r), TaggedSignalWriter::F64(w)) => w.write_all(&mut r),

            (r, TaggedSignalWriter::I8(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::I16(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::I32(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::I64(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::U8(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::U16(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::U32(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::U64(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::F32(w)) => w.write_all(&mut r.adapt_sample_type()),
            (r, TaggedSignalWriter::F64(w)) => w.write_all(&mut r.adapt_sample_type()),
        }
    }
}
